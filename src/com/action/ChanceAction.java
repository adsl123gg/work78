...
public class ChanceAction implements SessionAware{
	...
	public String addChance(){
		...
		messageService.add(user, user.getName()+"添加了机会"+chance.getName(),"机会" );
		
		if ("all".equals(optionsRadios)) {
			List<User> users=userService.getuOtherUsers();
			
			for (User user : users) {
				ChanceExtra chanceExtra=new ChanceExtra();
				chanceExtra.setUser(user);
				chanceExtra.setChance(chance);
				chanceExtraService.add(chanceExtra);
			}
	...
	public String updateChance(){
		...
		messageService.add(user, user.getName()+"更新了机会"+chance.getName(),"机会" );
	...
	public String addChancEvent(){
		...
		messageService.add(user, user.getName()+"添加了机会事件"+TextShorten.shorten(chanceEvent.getInfo()),"机会事件" );
	...
	public String updateChanceEvent(){
		...
		messageService.add(user, user.getName()+"更新了机会事件"+TextShorten.shorten(chanceEvent.getInfo()),"机会事件" );
	...
	public void delChanceEvent(){
		...
		messageService.add(user, user.getName()+"删除了机会事件"+TextShorten.shorten(chanceEvent.getInfo()),"机会事件" );
	...
	public String addChanceComment(){
		...
		messageService.add(user, user.getName()+"评论了机会事件"+TextShorten.shorten(chanceComment.getChanceEvent().getInfo()),"机会事件评论" );
	...
	public String delChanceComment(){
		messageService.add(user, user.getName()+"删除了对机会事件"+TextShorten.shorten(chanceCommentService.find(chanceComment_id).getChanceEvent().getInfo())+"的评论","机会事件评论" );
		
		chanceCommentService.delete(chanceComment_id);
		chanceComment=chanceCommentService.find(chanceComment_id);
	...
	public void changeChanceStatus() throws Exception{
		...
		messageService.add(user, user.getName()+"更改机会"+chance.getName()+"的状态为"+chance.getStatus(),"机会" );
	...
	public String addChanceType(){
		...
		messageService.add(user, user.getName()+"添加了机会类型"+chanceType.getName(),"机会类型" );
	...
	public String delChanceType(){
		try {
			String n=chanceTypeService.find(chanceTypeId).getName();
			chanceTypeService.delete(chanceTypeId);
			messageService.add(user, user.getName()+"删除了机会类型"+n,"机会类型" );
